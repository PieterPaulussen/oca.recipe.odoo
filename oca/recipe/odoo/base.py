...
class BaseRecipe(object):
    ...
    def install(self):
        ...
        dump_requirements = self.options.get('write-requirements-file')
        if dump_requirements:
            dump_requirements_file = self.make_absolute(dump_requirements)
            with open(dump_requirements_file, 'w+') as requirements_txt:
                requirements_txt.write(
                    '# this file is autogenerated, do not edit\n'
                    '# it exists for automated tools to analyze this '
                    'project\'s dependencies.\n'
                    '# if you need to add a dependency, do so in '
                    'buildout.cfg\n'
                )
                requirements_txt.writelines(sorted(
                    '%s==%s\n' % (egg_info.key, egg_info.version)
                    for egg_info in self.ws
                ))
            logger.info('Wrote versions to %s', dump_requirements)
